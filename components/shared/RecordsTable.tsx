      ...
      <table className="w-full min-w-[840px] text-sm">
        <thead>
          <tr className="border-b border-slate-200 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
            ...
            {showSubmitter && <th className="py-2 pr-4">{submitterLabel}</th>}
            <th className="py-2 pr-4 text-right">{quantityLabel}</th>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-b border-slate-100">
              ...
              {showSubmitter && <td className="py-2 pr-4 text-slate-600">{row.submitterName}</td>}
              <td className="py-2 pr-4 text-right text-slate-600">{row.quantity}</td>
