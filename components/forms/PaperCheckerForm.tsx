...
import { submitPaperCheckerRecord, type FormActionState } from '@/lib/paper-checker/actions';
import { paperCheckerTotal } from '@/lib/calculations';
...
export default function PaperCheckerForm({
  batches,
  subjects,
  ratesBySubject,
  staffId,
  ...
  const preview = useMemo(() => {
    ...
    if (!rate || !Number.isFinite(count) || count < 0 || studentCount === '') return null;

    return {
      rate,
      total: paperCheckerTotal(count, rate.perPaperPrice, rate.answerKeyPrice),
    };
    ...
    <form action={formAction} className="max-w-xl space-y-4">
      ...
      <div>
        ...
        <input
          ...
          type="text"
          required
          placeholder="e.g. Full Syllabus Test 3"
          className="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-500"
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700">Test marks (optional)</label>
          <input
            name="test_marks"
            type="number"
            min="0"
      ...
      <div className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
        {subjectId && !ratesBySubject[subjectId] && (
        ...
        {preview ? (
          <p className="text-slate-700">
            Estimated total:{' '}
            <span className="font-semibold text-slate-900">{preview.total.toFixed(2)}</span>{' '}
            ({preview.rate.perPaperPrice}/paper × students + {preview.rate.answerKeyPrice} answer
            key). The final amount is calculated by the server when you submit.
          </p>
        ) : (
