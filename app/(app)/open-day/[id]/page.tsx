...
import EditRecordForm from '@/components/open-day/EditRecordForm';
import {
  confirmOpenDayRecord,
  rejectOpenDayRecord,
  approveOpenDayRecord,
  markOpenDayRecordPaid,
...
export default async function OpenDayDetailPage({
  params,
}: {
  ...
  const [{ data: subject }, { data: batch }, { data: branch }, { data: submitter }, { data: confirmedBy }, { data: approvedBy }] =
    await Promise.all([
      record.subject_id
      ...
      supabase.from('profiles').select('full_name').eq('id', record.staff_id).single(),
      record.confirmed_by
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">{record.event_name}</h1>
      ...
      <dl className="mt-6 grid grid-cols-2 gap-4 rounded-md border border-slate-200 p-4 sm:grid-cols-3">
        <Field label="Event date" value={record.event_date} />
      ...
      <div className="mt-6 space-y-4">
        {canConfirmReject && (
          <ConfirmRejectPanel
            confirmAction={confirmOpenDayRecord.bind(null, record.id)}
            rejectAction={rejectOpenDayRecord.bind(null, record.id)}
          />
        )}
        {isAdmin && (
          <ApprovePaidPanel
            paymentStatus={record.payment_status}
            approveAction={approveOpenDayRecord.bind(null, record.id)}
            markPaidAction={markOpenDayRecordPaid.bind(null, record.id)}
          />
        )}
        {editContext && (
          <EditRecordForm
